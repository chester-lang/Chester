...
@js.native
trait FnCallNameOptionsFn extends StObject {
  ...
  def apply(options: Unit, fn: TestFn): js.Promise[Unit] = js.native
  def apply(options: TestOptions): js.Promise[Unit] = js.native
  def apply(options: TestOptions, fn: TestFn): js.Promise[Unit] = js.native
}
