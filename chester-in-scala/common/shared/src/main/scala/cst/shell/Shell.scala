...
trait Shell {
  ...
  def captureCondition(command: Command, muteStdout: Boolean = false, muteStderr: Boolean = false): Condition
  ...
  def store[T <: Val](variable: Var[T], value: T)(using tag: ValType[T]): Statement
  ...
  def ifThenElse(cond: Condition, thenBranch: Statement, elseBranch: Option[Statement]): Statement
...
class ShellMonad(using val shell: Shell) {
  ...
  case class FStrStore(variable: shell.Var[shell.Str], value: shell.Str) extends F[shell.Str] {
    override def flatMap[U](f: shell.Str => F[U]): F[U] = FStatementAppend(shell.store(variable, value), f(variable))
  }
  ...
  case class FIntStore(variable: shell.Var[shell.IntNum], value: shell.IntNum) extends F[shell.IntNum] {
    override def flatMap[U](f: shell.IntNum => F[U]): F[U] = FStatementAppend(shell.store(variable, value), f(variable))
  }

  def FStore[T <: shell.Val](variable: shell.Var[T], value: T)(using tag: shell.ValType[T]): F[T] = tag match {
  ...
  implicit class FStatement(value: shell.Statement) extends F[Unit] {
    override def flatMap[U](f: Unit => F[U]): F[U] = FStatementAppend(value, f(()))
  }

  case class FStatementAppend[T](value: shell.Statement, result: F[T]) extends F[T] {
    override def flatMap[U](f: T => F[U]): F[U] = FStatementAppend(value, result.flatMap(f))
  }
