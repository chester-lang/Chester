...
case class TyckResult0[Problem <: WithServerity, +S, +T](state: S, result: T, problems: Vector[Problem] = Vector()) {
...
object TyckResult {
  ...
  object Failure {
    def unapply[S, T](x: TyckResult[S, T]): Option[(Vector[TyckError], Vector[TyckWarning], S, T)] = {
      if (!x.errorsEmpty) Some((x.problems.filter(_.isInstanceOf[TyckError]).asInstanceOf[Vector[TyckError]], x.problems.filter(_.isInstanceOf[TyckWarning]).asInstanceOf[Vector[TyckWarning]], x.state, x.result))
      else None
    }
  }
}
