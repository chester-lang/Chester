...
import chester.tyck.api.{
  NoopSemanticCollector,
  SemanticCollector,
  UnusedVariableWarningWrapper
}
...
trait ProvideElaborater
    extends ProvideCtx
    with Elaborater
    with ElaboraterFunction
    with ElaboraterFunctionCall {
  ...
  def elabBlock(expr: Block, ty0: CellIdOr[Term], effects: CIdOf[EffectsCell])(
      using
      localCtx: LocalCtx,
      parameter: SemanticCollector,
      ck: Tyck,
  ...
  def finalizeJudge(
    ...
    boundary {
      while (true) {
        ...
        judge = judge.replaceMeta(x =>
          able.readUnstable(x.unsafeRead[CellId[Term]]).get
        )
      }
    }
    judge
