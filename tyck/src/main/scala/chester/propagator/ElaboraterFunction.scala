...
trait ElaboraterFunction extends ProvideCtx with Elaborater {
  def elabFunction(expr: FunctionExpr, ty: CellId[Term], outerEffects: CellId[Effects])(using ctx: LocalCtx, parameter: Global, ck: Ck, state: StateAbility[Ck]): Term
}

trait ProvideElaboraterFunction extends ElaboraterFunction {
  def elabArg(arg: Arg, effects: CellId[Effects])(using localCtx: MutableLocalCtx, parameter: Global, ck: Ck, state: StateAbility[Ck]): ArgTerm = {
  ...
  def elabTelescope(telescope: DefTelescope, effects: CellId[Effects])(using localCtx: MutableLocalCtx, parameter: Global, ck: Ck, state: StateAbility[Ck]): TelescopeTerm = {
