...
object ColorfulToHtml {
  ...
  def styleToCss(style: Style): String = {
    val fgColor = style.foreground.map(fg => s"color: ${foregroundToCss(fg)};").getOrElse("")
    val bgColor = style.background.map(bg => s"background-color: ${backgroundToCss(bg)};").getOrElse("")
