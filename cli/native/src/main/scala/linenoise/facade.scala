...
object facade {
  ...
  def setMultiline(enabled: Boolean): Unit = linenoiseSetMultiLine(if (enabled) 1 else 0)

  /** Sets a completion callback function. */
  // TODO

  /** Adds a completion string to the list of completions. */
  def addCompletion(completions: Ptr[linenoiseCompletions], completion: String): Unit = Zone {
